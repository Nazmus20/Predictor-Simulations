function [pred_time, x_pred, L, z_tilde_l] = ...
...
for k=1:length(Sim.t_vec)-Sim.in_delDT-1%-Sim.out_delDT   
    n = k-d1; l = k-d1-d2;
    if n>=1 && Sim.t_vec(k)<0
        x_hat_l_plus_one(:,n) = Sim.initial_state;
        %x_hat_l_plus_one(4,n) = Sim.initial_state(4)-.2303;
    ...
    if Sim.t_vec(k)>=0
        ...
        %z_hat_l(4,l) = z_hat_l(4,l) +.0470 ;
        %Measurement prediction error (When C = I, measurement = state)
        z_tilde_l(:,l) = H*available_states(:,k+Sim.in_delDT+1) - z_hat_l(:,l);
        ...
        plot(Sim.t_vec(k-Sim.in_delDT-1), available_states(2,k), 'k+', Sim.t_vec(k), z_hat_l(2,l), 'ro')
        ...
        x_pred(:,iter) = F^(d1+d2+1)*(x_hat_l_plus_one(:,n)) + mtx_sum ;%+ L(:,:,n)*z_tilde_l(:,l);
...
ylabel('East, y (m)')
legend('Measured, $y_l$ (m)', 'Estimated, $\hat{y}_{l|l}$ (m)', 'Interpreter', 'latex', 'FontSize', 12)

figure
    plot(pred_time, z_tilde_l(4,:))
    xlabel('Time, t(s)')
