%Script to run the UAV steady translation motion along y-axis. That
...
%XD(:,16/Sim.Ts+1:end) = zeros(size(XD(:,16/Sim.Ts+1:end)));
...
Q = 1*eye(12); Q(3,3) = 100000; R = 1*eye(4);
...
%uNL = [uNL_append, uNL];
 ...
 %%  
 clear all; clc; close all;
 load workspace1Hz.mat %Temporary, load all workspace variable to reduce time
...
plot(orig_time-Sim.in_del-Sim.out_del-Sim.Ts-Sim.Ts, orig_measurement(int,:), 'k-', t_KP1, predicted_states_KP1(int,:), 'r-', t_KP1, predicted_states_SP(int,1:end-1), 'b-')
