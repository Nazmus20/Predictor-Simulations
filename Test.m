...
%Create a subsystem excluding x and y
Asub = A(3:12, 3:12); Bsub = B(3:12,:); Csub = eye(10); Dsub =[];
sysCTsub = ss(Asub, Bsub, Csub, Dsub)
...
%The steady state values given above are the initial values (that is, we
%are starting from steady motion) FOR THE SUBSYSTEM!
x_vec_sub0 = x_vec0(3:12);
...
Q = eye(12); R = eye(4); Qsub = eye(10); Rsub = eye(4);
%Find the CT LQR gain
Klqr = lqr(sysCT, Q, R); Klqr_sub = lqr(sysCTsub, Qsub, Rsub);

%Apply outgoing delay to the references in the subsystem. Due to this delay between the
%groundstation and the UAV the references will reach the UAV after some
%delay
del_ref_vec = addDelay(t_vec, ref_vec, out_del, x_vec0);
del_ref_vec_sub = addDelay(t_vec, ref_vec_sub, out_del, x_vec_sub0);
