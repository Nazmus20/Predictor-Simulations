...
%Debug case lateral translation and hold !!!!!!!!!!!!!!!!!
XD(2,4/Sim.Ts+1:end) = XD(2,4/Sim.Ts+1:end)+10;

%XD(2,10/Sim.Ts+1:end) = XD(2,10/Sim.Ts+1:end)+5;
...
Q = 1*eye(12); Q(3,3) = 100; Q(4,4) = 100; Q(8,8) = 100; R = 1*eye(4);
...
figure
plot(Sim.t_vec, Sim.des_state(8,:), 'k-', Sim.t_vec, ...
    Sim.del_des_state(8,:), 'k--', Sim.t_vec, ...
    actual_states_NL(:,8), 'r-', Sim.t_vec, ...
    actual_states_L(:,8), 'b--', Sim.t_vec, ...
    del_actual_states_NL(8,:), 'm-', Sim.t_vec, ...
    del_actual_states_L(8,:), 'c--')
...
figure
plot(Sim.t_vec, Sim.des_state(4,:), 'k-', Sim.t_vec, ...
    Sim.del_des_state(4,:), 'k--', Sim.t_vec, ...
    actual_states_NL(:,4), 'r-', Sim.t_vec, ...
    actual_states_L(:,4), 'b--', Sim.t_vec, ...
    del_actual_states_NL(4,:), 'm-', Sim.t_vec, ...
    del_actual_states_L(4,:), 'c--')
...
plot(Sim.t_vec, Sim.des_state(3,:), 'k-', Sim.t_vec, ...
    Sim.del_des_state(3,:), 'k--', Sim.t_vec, ...
    predicted_states_SP(3,:), 'b-', Sim.t_vec, ...
    del_actual_states_NL(3,:), 'r-')
...
plot(orig_time-Sim.in_del-Sim.out_del-Sim.Ts, orig_measurement(2,:), 'k-', t_KP2, predicted_states_KP2(2,:), 'r-', t_KP2, predicted_states_SP(2,1:end-1), 'b-')
...
for iter = 1:length(time_vector)
    ...
    plot([time_vector(iter), time_vector(iter)], [-kalman_vec(3,iter), -actual_vec(3,iter)], 'r:', ...
        [time_vector(iter), time_vector(iter)], [-smith_vec(3,iter), -actual_vec(3,iter)], 'b:')
    hold on
end

plot(time_vector, -actual_vec(3,:), 'k-', time_vector, -kalman_vec(3,:), 'r-', time_vector, -smith_vec(3,:), 'b-')
fig = get(gca, 'children');
legend([fig(1), fig(2), fig(3), fig(4), fig(5)], {'SP', 'KP', 'Measurement shifted', 'SP err', 'KP err'}, 'Location', 'southeast')
xlabel('Time, t (s)')
ylabel('Height, -z (m)')

err_kalman = kalman_vec-actual_vec; err_smith = smith_vec-actual_vec;
KP_norm = norm(err_kalman(3,:))/length(time_vector)
SP_norm = norm(err_smith(3,:))/length(time_vector)
