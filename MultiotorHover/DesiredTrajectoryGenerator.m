...
var = [u_TD; u_phiD; u_thetaD; u_psiD];
FUNC = [EQN(9:12)]; %The other Eqns are 0=0 so they are ignored
ht = matlabFunction(FUNC, 'Vars', {var});
%Rewrite them in 'fsolve()' required format

var_val = fsolve(ht, [-1; -1; -1; -1]);

%Create the reference trajectories
ref_states = des_vals(1:12)*ones(size(Sim.t_vec));
ref_state_derivs = des_vals(17:end)*ones(size(Sim.t_vec));
ref_inputs = var_val*ones(size(Sim.t_vec));
