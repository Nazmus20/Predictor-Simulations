...
%yaw angle and then hovers again. !!!!!!!!!!THIS IS A BACKUP COPY SHOWING
%HOW THE KP FAILS TO PREDICT!!!!!!!!! ALSO WHEN THE SAMPLING TIME IS
%DECREASED TO .1 s THE SP ALSO FAILS TO PREDICT
...
Sim.t_vec = 0:Sim.Ts:15; %Creating the time vector, [s]
...
predicted_states_SP = ...
    SmithPredictor(sysDT, Sim, del_actual_states_NL, Klqr);

%Figure with true trajectories and predicted trajectories
figure
plot(Sim.t_vec, Sim.des_state(3,:), 'k-', Sim.t_vec, ...
    Sim.del_des_state(3,:), 'k--', Sim.t_vec, ...
...
t_vec_append = -2:Sim.Ts:0-Sim.Ts;
...
[predicted_states_KP, ulqr, u_est] = ...
    KalmanPredictor(sysDT, Sim, del_actual_states_NL, uNL, Klqr);

figure
plot(Sim.t_vec, del_actual_states_NL(3,:), 'k-', Sim.t_vec(1:end-1), predicted_states_KP(3,:), 'r--')
title("KP")
figure
plot(Sim.t_vec(1:end-1),ulqr(1,:), 'b--', Sim.t_vec(1:end-1), u_est(1,:)+Values.steady_inputs(1), 'r--')
